...
const endOfTurn = () => {
  ...
  if (
    (col1.textContent === currentPlayer().symbol &&
      col2.textContent === currentPlayer().symbol &&
      col3.textContent === currentPlayer().symbol) ||
    (col4.textContent === currentPlayer().symbol &&
      col5.textContent === currentPlayer().symbol &&
      col6.textContent === currentPlayer().symbol) ||
    (col7.textContent === currentPlayer().symbol &&
      col8.textContent === currentPlayer().symbol &&
      col9.textContent === currentPlayer().symbol) ||
    (col1.textContent === currentPlayer().symbol &&
      col4.textContent === currentPlayer().symbol &&
      col7.textContent === currentPlayer().symbol) ||
    (col2.textContent === currentPlayer().symbol &&
  ...
  // If all of the tiles have a sign but none of the players managed to place three of them in a line, it is a tie
    ...
    col1.textContent.length !== 0 &&
    col2.textContent.length !== 0 &&
    col3.textContent.length !== 0 &&
    col4.textContent.length !== 0 &&
    col5.textContent.length !== 0 &&
    col6.textContent.length !== 0 &&
    col7.textContent.length !== 0 &&
    col8.textContent.length !== 0 &&
    col9.textContent.length !== 0 &&
    playerOne.isCurrentPlayer !== 'winner' &&
    playerTwo.isCurrentPlayer !== 'winner'
  ) {
    tie();
...
formEnterNames.addEventListener('input', () => {
  ...
  if (inputPlayerOneUsername.value.length === 0 || inputPlayerTwoUsername.value.length === 0) {
    ...
    (inputPlayerOneUsername.value.length > 0 &&
      inputPlayerTwoUsername.value.length > 0 &&
      checkA.checked) ||
    (inputPlayerOneUsername.value.length > 0 &&
      inputPlayerTwoUsername.value.length > 0 &&
      checkB.checked)
